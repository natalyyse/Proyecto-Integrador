...
class PromocionDePlato(models.Model):
    ...
    def aplicar_descuento(self):
        if self.precio_descuento:
            ...
            self.precio_descuento = precio_original - self.precio_descuento

    def save(self, *args, **kwargs):
        # Aplica el descuento antes de guardar
...
class RegistroDeVenta(models.Model):
    ...
    cantidad_plato = models.PositiveIntegerField(null=True, blank=True)
    bebida = models.ForeignKey('Bebida', on_delete=models.SET_NULL, null=True, blank=True)
    cantidad_bebida = models.PositiveIntegerField(null=True, blank=True)
    entrada = models.ForeignKey('Entrada', on_delete=models.SET_NULL, null=True, blank=True)
    cantidad_entrada = models.PositiveIntegerField(null=True, blank=True)
    ...
    def save(self, *args, **kwargs):
        super().save(*args, **kwargs)
        GananciaMe.actualizar_o_crear_ganancia_mes(self.fecha_venta)
...
class ReservaDeMesa(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        super().save(*args, **kwargs)
        GananciaMe.actualizar_o_crear_ganancia_mes(self.fecha_reg)



class GananciaMe(models.Model):
