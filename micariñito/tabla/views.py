...
from .models import Usuario, Categoria, NotificacionMovil, ReservaDeMesa, Plato, PromocionDePlato, ComentarioCalificacion, Cliente, RegistroDeVenta, Bebida, Entrada, Contactano, GananciaMe
...
class ClienteViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        queryset = Cliente.objects.all()
        email = self.request.query_params.get('correo_electronico', None)
        if email is not None:
            queryset = queryset.filter(correo_electronico=email)
        return queryset
...
class ContactoViewSet(viewsets.ModelViewSet):
    queryset = Contactano.objects.all()
    serializer_class = ContactoSerializer

class GananciaMesViewSet(viewsets.ModelViewSet):
    queryset = GananciaMe.objects.all()
...
def descargar_reporte_pdf(request, pk):
    ganancia_mes = get_object_or_404(GananciaMe, pk=pk)
